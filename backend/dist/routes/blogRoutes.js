...
function classifyDifficulty(title, description) {
...
/* Academic weighting */
function academicScore(year, difficulty) {
    const map = {
...
/* Remove duplicates */
function removeDuplicates(blogs) {
    const seen = new Set();
...
/* Ranking */
function computeScore(blog, year) {
    const academicWeight = academicScore(year, blog.difficulty);
    const popularity = (blog.public_reactions_count || 0) * 0.6 + (blog.comments_count || 0) * 0.4;
    return academicWeight * 0.4 + popularity * 0.6;
}
router.get("/", async (req, res) => {
    try {
        const year = typeof req.query.year === "string" ? req.query.year : "FE";
        const response = await axios.get("https://dev.to/api/articles?tag=technology&per_page=40");
        let blogs = response.data.map((blog) => {
