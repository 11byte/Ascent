...
export default function FinalDataStream({ domain = "Cybersecurity", onComplete }: { domain?: string, onComplete?: () => void }) {
  ...
  const handleBubbleClick = (bubble: any, e: React.MouseEvent) => {
    ...
    if (bubble.isTarget) {
      ...
      setScore(s => {
        const newScore = s + 1;
        if (newScore >= targetScore) setTimeout(() => setGameWon(true), 500);
        return newScore;
      });
      // Add Cyan blast
          ...
          {gameWon && (
            ...
            >
              <div className="bg-gray-900/90 border border-green-500/50 p-10 rounded-3xl shadow-[0_0_60px_rgba(34,197,94,0.3)] text-center max-w-md">
                ...
                <button onClick={onComplete || (() => alert("Go to next phase!"))} className="w-full group bg-gradient-to-r from-green-400 to-cyan-500 hover:from-green-300 hover:to-cyan-400 text-gray-900 font-bold py-4 px-8 rounded-xl flex items-center justify-center gap-2 transition-all hover:scale-[1.03] active:scale-[0.97] shadow-lg">
                  Next Challenge 
